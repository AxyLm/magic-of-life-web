...
const spvModel = require('../../../db/model/spvModel')
const multer = require('multer')
const fs = require('fs')
const path = require("path")
// let common = require('../../../../cakeShop-node/utils/com.js')
...
let storage = multer.diskStorage({
    destination: function (req, file, cb) {
        // 指定文件路径
        cb(null, path.join(__dirname, './../../../static/scripturepavilion'))
    },
...
/**
 * @api {post} /scripturepavilion/upload  文件上传
 * @apiGroup File
 *
...
Router.post('/upload', upload.single('scrpture'), (req, res) => {
    ...
    if (types.indexOf(tmp) == -1) {
        fs.unlink(path.join(__dirname, './../../../static/scripturepavilion/' + req.file.filename), (err) => {
        })
